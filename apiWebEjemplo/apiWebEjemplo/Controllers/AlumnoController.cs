...
namespace apiWebEjemplo.Controllers
{
    ...
    public class AlumnoController : ApiController
    {
        ...
        [Route("api/alumno/eliminar/{codigo}")]
        [HttpDelete]
        ...
        [Route("api/alumno/editar")]
        [HttpPut]
        public ResponseServer editar(Alumno alumno)
        {
            ResponseServer rptaServidor = new ResponseServer();
            ServiceAlumno servicioAlumno = new ServiceAlumno();
            int procesar = servicioAlumno.operacionAlumno("editar", alumno);
            if (procesar < 1)
            {
                rptaServidor.mensaje = "Hubo un error.";
                rptaServidor.codigo = 0;
